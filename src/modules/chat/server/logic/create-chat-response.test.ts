...
vi.mock("../../logic/save-conversation", async () => {
  const actual = await vi.importActual<typeof import("../../logic/save-conversation")>(
...
function getLastStreamInput() {
  if (!state.lastStreamInput) {
    throw new Error("Expected createAgentUIStreamResponse to receive stream options");
  }
...
describe("createChatResponse", () => {
  ...
  test("should close MCP tools and return a generic error message when the stream errors", async () => {
    ...
    expect(onError()).toBe("Something went wrong while generating the response.");

...
export function buildOreAuthOptions(): BetterAuthOptions {
  const secret = readRequiredAuthEnv("BETTER_AUTH_SECRET", env.BETTER_AUTH_SECRET);
  const baseURL = readRequiredAuthEnv("BETTER_AUTH_URL", env.BETTER_AUTH_URL);
  const turnstileSecretKey = readRequiredAuthEnv("TURNSTILE_SECRET_KEY", env.TURNSTILE_SECRET_KEY);

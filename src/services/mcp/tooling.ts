...
async function closeMcpClient(client: Awaited<ReturnType<typeof createMCPClient>>) {
...
function toRequest(requestInfo: RequestInfo | URL, init?: RequestInit): Request {
  return requestInfo instanceof Request ? requestInfo : new Request(requestInfo, init);
}
...
function createTransportFetch(
  ...
  if (!serviceBinding || isLoopbackHost(serverUrl.hostname)) {
    return async (requestInfo, requestInit) => fetch(toRequest(requestInfo, requestInit));
  }

...
def prepare_dev_vars(repo_root: Path, backup_dir: Path, warnings: list[str]) -> dict:
	...
	source_path = target_path if target_path.exists() else repo_root / ".dev.vars.example"
	content = read_text(source_path)

	session_access_secret = read_existing_env_value(content, "SESSION_ACCESS_SECRET")
	if not session_access_secret or session_access_secret == "change-me-local-dev-secret":
		session_access_secret = secrets.token_hex(32)
		warnings.append("Generated temporary SESSION_ACCESS_SECRET for local UI testing.")

	message_integrity_secret = read_existing_env_value(
		content, "MESSAGE_INTEGRITY_SECRET"
	)
	if (
		not message_integrity_secret
		or message_integrity_secret == "change-me-local-message-integrity-secret"
	):
		message_integrity_secret = secrets.token_hex(32)
		warnings.append(
			"Generated temporary MESSAGE_INTEGRITY_SECRET for local UI testing."
		)
	...
	content = upsert_env_value(
		content, "SESSION_ACCESS_SECRET", session_access_secret
	)
	content = upsert_env_value(
		content, "MESSAGE_INTEGRITY_SECRET", message_integrity_secret
	)
...
def replace_turnstile_site_key(content: str) -> tuple[str, bool]:
	...
	replaced_content, count = pattern.subn(
		rf'\g<1>{TURNSTILE_SITE_KEY}\g<3>', content
	)
...
def prepare_wrangler_config(
	...
	source_path = target_path if target_path.exists() else repo_root / "wrangler.jsonc.example"
	content = read_text(source_path)
